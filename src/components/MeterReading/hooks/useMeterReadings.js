...
export const useMeterReadings = () => {
  ...
  const loadMeterReadings = useCallback(async (propId, rId, maxRetries = 3) => {
  ...
  const loadRoomDataForSPA = useCallback(async (propId, rId) => {
    try {
      const currentGasUrl = gasWebAppUrl || sessionStorage.getItem('gasWebAppUrl');
      const fetchUrl = `${currentGasUrl}?action=getMeterReadings&propertyId=${propId}&roomId=${rId}`;
      const response = await fetch(fetchUrl);
  ...
  return {
    ...
    setError, setLoading,
    loadMeterReadings, loadRoomDataForSPA, mapNavigationReadingsData,
  };
};
