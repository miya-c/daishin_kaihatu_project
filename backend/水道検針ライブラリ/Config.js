...
function setConfig(path, value) {
  try {
    ...
    Logger.log(`設定更新エラー: ${error.message}`);
    return false;
  }
}
 ...
 * @returns {Object} 環境設定
 */
function loadEnvironmentConfig(ss = null) {
  if (!ss) {
  ...
  try {
    ...
    if (!settingsSheet) {
      Logger.log('設定シートが見つからないため、デフォルト設定を使用します');
      return CONFIG;
    }
    
    const data = settingsSheet.getDataRange().getValues();
    const envConfig = JSON.parse(JSON.stringify(CONFIG)); // 深いコピー
    
    // 設定値を読み込み
    for (let i = 1; i < data.length; i++) {
      const [key, value] = data[i];
      if (key && value !== undefined && value !== '') {
        setConfigValue(envConfig, key, value);
      }
    }
    
    return envConfig;
  } catch (error) {
    Logger.log(`環境設定読み込みエラー: ${error.message}`);
    return CONFIG;
  }
}
...
function setConfigValue(config, key, value) {
  try {
    ...
    Logger.log(`設定値設定エラー: ${error.message}`);
  }
}
