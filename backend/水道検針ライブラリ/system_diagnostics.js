...
function showSystemDiagnostics(diagnostics = null) {
  try {
    ...
    message += `状態: ${getStatusIcon(diagnostics.status)} ${diagnostics.status.toUpperCase()}\n\n`;
...
function showIntegrationSummary() {
  try {
    ...
    if (ui) {
      ...
      message += `システム状態: ${getStatusIcon(summary.diagnostics.status)} ${summary.diagnostics.status.toUpperCase()}\n\n`;
