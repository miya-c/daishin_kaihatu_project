...
function runLibraryTest() {
  ...
  // テスト5: generateRoomIds関数存在チェック
  try {
    const functionCheck = checkFunction('generateRoomIds');
    results.tests.push({
      name: "checkFunction(generateRoomIds)",
      status: functionCheck.exists ? "PASS" : "FAIL",
      result: `関数存在: ${functionCheck.exists}`
    });
    if (functionCheck.exists) results.summary.passed++;
  ...
  try {
    ...
    results.tests.push({
      ...
      status: "PASS",
      result: `利用可能関数数: ${availableFunctions.length}`
    });
    results.summary.passed++;
  } catch (error) {
    results.tests.push({
  ...
  // テスト7: 標準偏差計算（整数値確認）
  try {
    const testValues = [10, 12, 14, 16, 18];
    const stdDev = calculateSTDEV_S(testValues);
    const isInteger = Number.isInteger(stdDev);
    results.tests.push({
      name: "calculateSTDEV_S(integer)",
      status: isInteger ? "PASS" : "FAIL",
      result: `標準偏差: ${stdDev}, 整数: ${isInteger}`
    });
    if (isInteger) results.summary.passed++;
    else results.summary.failed++;
  } catch (error) {
    results.tests.push({
      name: "calculateSTDEV_S(integer)",
  ...
  // テスト8: 初期検針データ作成（標準偏差整数化確認）
  try {
    // createInitialInspectionData関数が利用可能かチェック
    const functionCheck = checkFunction('createInitialInspectionData');
    results.tests.push({
      name: "createInitialInspectionData(available)",
      status: functionCheck.exists ? "PASS" : "FAIL",
      result: `関数存在: ${functionCheck.exists}`
    });
    if (functionCheck.exists) results.summary.passed++;
  ...
  // テスト9: 標準偏差の数式が整数化されているかチェック
  try {
    const checkResult = checkStandardDeviationIntegerFormat();
    results.tests.push({
      name: "checkStandardDeviationIntegerFormat",
      status: checkResult.success ? "PASS" : "FAIL",
      result: checkResult.message || checkResult.error
    });
    if (checkResult.success) results.summary.passed++;
    else results.summary.failed++;
  } catch (error) {
    results.tests.push({
      name: "checkStandardDeviationIntegerFormat",
